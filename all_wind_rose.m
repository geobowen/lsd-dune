function varargout = all_wind_rose(D,F,varargin)
...
lineColors=[65/255 173/255 203/255];
borderColor='none';
onAxes=false;
iflip=0;
inorm=0;
...
for i=1:length(vin)
  ...
  elseif isequal(vin{i},'lablegend')
    legStr=vin{i+1};
  elseif isequal(vin{i},'lablegend1')
    legStr1=vin{i+1};
...
handles(end+1)=plot([-g-ri -ri nan ri g+ri nan 0 0 nan 0 0],...
                    [0 0 nan 0 0 nan -g-ri -ri nan ri g+ri],'--','linewidth',2,'color',lineColors);
t0=[0:360]*pi/180;
labs=[];
Ang=[1/4 3/4 5/4 7/4]*pi;
Valign={'bottom' 'bottom' 'top' 'top'};
Halign={'left' 'right' 'right' 'left'};
for i=1:ncircles
  x=(ci(i)+ri)*cos(t0);
  y=(ci(i)+ri)*sin(t0);
  ...
  x1=ri*cos(t0);
  y1=ri*sin(t0);
  circles(i)=plot(x,y,'--','linewidth',2,'color',lineColors);
  ...
  labs(i)=text((ci(i)+ri)*cos(Ang(quad)),(ci(i)+ri)*sin(Ang(quad)),[num2str(ci(i)),'%'],...
      ...
      'Fontsize',18,'FontName', 'Times','BackgroundColor',percBg);
end
...
% N S E W labels:
bg='none';
args={'BackgroundColor',bg,'FontSize',18,'FontName', 'Times'};
% h(1)=text(-g-1.5*ri, 0,'270', 'VerticalAlignment','middle','HorizontalAlignment','right', args{:});
% h(2)=text( g+1.5*ri, 0,'90', 'VerticalAlignment','middle','HorizontalAlignment','left',  args{:});
% h(3)=text( 0,-g-ri,'180','VerticalAlignment','top','HorizontalAlignment','center', args{:});
% h(4)=text( 0, g+ri,'0','VerticalAlignment','bottom','HorizontalAlignment','center', args{:});
% h(1)=text( 0, g+ri,'North','VerticalAlignment','bottom','HorizontalAlignment','center', args{:});
...
if legType==1 % contimuous.
  for j=1:length(Ag)-1
    ...
    y1=y0+h;
    %%%%  Decommenter pour la colormap
    handles(end+1)=fill([x0 x1 x1 x0],[y0 y0 y1 y1],cor{j});
    if mod(j,2)~=0
    handles(end+1)=text(x1+L/4,y0,lab,'VerticalAlignment','middle','fontsize',25);
    end
    y0=y1;
  end
  if ~ (hasH & IncHiLow)
...
elseif legType==2 % separated boxes.
  for j=1:length(Ag)-1
    ...
    if mod(j,2)~=0
    handles(end+1)=text(x1+L/4,(y0+dy+y1)/2,lab,'VerticalAlignment','middle','fontsize',25);
    end
    y0=y1;
  end
    ...
    y1=y0+h;
    handles(end+1)=fill([x0 x1 x1 x0],[y0+dy y0+dy y1 y1],'w','edgecolor','w');
    end
end
...
handles(end+1)=text(0,-g-ri,legStr,'HorizontalAlignment','center','VerticalAlignment','top','fontsize',20,'FontName', 'Times');

handles(end+1)=text(x,y,legStr1,'HorizontalAlignment','left','VerticalAlignment','bottom','fontsize',20,'FontName', 'Times');
